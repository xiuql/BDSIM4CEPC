#ifndef BDSSTEPPERBASE_H
#define BDSSTEPPERBASE_H
...
/**
 * @brief All BDSIM steppers require the ability to convert from
 * global to local coordinates. The prescribed method to do this
 * is by using a G4Navigator instance. There is of course the main
 * navigator for tracking, but requesting information on a global
 ...
 * class can use. This is not done to reduce memory usage but because
...
class BDSStepperBase: public G4MagIntegratorStepper
{
public:
  BDSStepperBase(G4Mag_EqRhs* eqRHS, G4int nVariables);

  static void AttachWorldVolumeToStepper(G4VPhysicalVolume* worldPV);
