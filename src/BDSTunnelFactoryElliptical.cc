...
BDSGeometryComponent* BDSTunnelFactoryElliptical::BuildTunnelSection(G4String      name,
								     G4double      length,
								     ...
								     G4double      tunnel2)
{
  ...
  TestInputParameters(length, tunnelThickness, tunnelSoilThickness, tunnelMaterial,
		      tunnelSoildMaterial, tunnelFloorOffset, tunnel1, tunnel2);

  // build the solids
  G4VSolid* tunnelInner, tunnelOuter;
  tunnelInner = new G4EllipticalTube(name + "_tunnel_inner_solid", // name
				     tunnel1,                      // x half width
				     tunnel2,                      // y half width
				     length); // half length - long for unambiguous subtraction

  tunnelOuter = new G4EllipticalTube(name + "_tunnel_outer_solid", // name
				     tunnel1 + tunnelThickness,    // x half width
				     tunnel2 + tunnelThickness,    // y half width
				     (length*0.5)-2*lengthSafety); // half length

  tunnelSolid = new G4SubtractionSolid(name + "_tunnel_solid", // name
				       tunneOuter,
				       tunneInner);            // outer minus inner

  G4VSolid* soilInner, soilOuter;
  soilInner = new G4EllipticalTube(name + "_soil_inner_solid",               // name
				   tunnel1 + tunnelThickness + lengthSafety, // x half width
				   tunnel2 + tunnelThickness + lengthSafety, // y half width
				   length); // half length - long for unambiguous subtraction

  soilOuter = new G4EllipticalTube(name + "_soil_outer_solid",                               // name
				   tunnel1 + tunnelThickness + soilThickness + lengthSafety, // x half width
				   tunnel2 + tunnelThickness + soilThickness + lengthSafety, // y half width
				   (length*0.5)-2*lengthSafety); // half length

  soilSolid = new G4SubtractionSolid(name + "_tunnel_solid", // name
				       tunneOuter,
				       tunneInner);            // outer minus inner
...
BDSBeamPipe* BDSTunnelFactoryElliptical::CreateBeamPipeAngledInOut(G4String    nameIn,              // name
...
BDSBeamPipe* BDSTunnelFactoryElliptical::CommonFinalConstruction(G4String    nameIn,
								   ...
								   G4double    beamPipeThicknessIn)
{
  ...
  BDSTunnelFactoryBase::CommonConstruction(nameIn,
					     vacuumMaterialIn,
					     beamPipeMaterialIn,
					     lengthIn);
...
void BDSTunnelFactoryElliptical::CreateGeneralAngledSolids(G4String      nameIn,
							     G4double      lengthIn,
							     G4double      aper1In,
							     G4double      aper2In,
							     G4double      beamPipeThicknessIn,
